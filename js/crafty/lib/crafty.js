...
(function (window, initComponents, undefined) {
    ...
    * select entities by their components.
    *
    * If there is more than one match, the return value is an Array-like object
    * listing the ID numbers of each matching entity. If there is exactly one
    * match, the entity itself is returned. If you're not sure how many matches
    * to expect, check the number of matches via Crafty(...).length.
    * Alternatively, use Crafty(...).each(...), which works in all cases.
    *
    * @example
    * ~~~
    *    Crafty("MyComponent")
    ...
    * ~~~
    * 
    * The first selector will return all entities that have the component
    * `MyComponent`. The second will return all entities that have `Hello` and
    * `2D` and `Component` whereas the last will return all entities that have
    * at least one of those components (or).
    *
    * ~~~
    *   Crafty("*")
    * ~~~
